...
* 包装类和基本数据类型的转换(以int与Integer转换为例)
  ...
  > 若不在数字范围 则返回的是新不同的对象
  >
  > 2. Integer i = 127; int j = 127;  System.out.printf( i == j ) //相等 有基本数据类型就判断值相等
...
* String  StringButter  StringBuilder 的区别
...
> 6. 对字符串操作的效率 StringBuilder > StringButter > String
...
>```java
>//3. binarySearch 方法
>//对有序数组进行查找 存在返回下标 不存在返回应该存在的位置 + 1
>int[] arr = {1,2,3,4,5,6};
